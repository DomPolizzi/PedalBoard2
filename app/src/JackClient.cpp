...
bool JackClient::start()
{
	...
	if(!running && callback)
	{
		//Connect to server using modern API.
		jack_status_t status;
		jackClient = jack_client_open(name.c_str(), JackNullOption, &status);
		if(!jackClient)
			cout << "JackClient: Could not connect to Jack Server. Status: " << status << endl;
		else
		{
